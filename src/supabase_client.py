...
def get_sales(since: str, until: str) -> pd.DataFrame:
    ...
    Columnas: ad_id, valor, fecha_compra (tz-aware en UTC).
    """
    ...
    if not df.empty:
        ...
        # Forzamos tz-aware UTC. Los consumidores decidirán a qué zona convertir.
...
def get_contactos(since: str, until: str) -> pd.DataFrame:
    ...
    while True:
        result = (
            ...
            .select("primer_ad_id, primer_contacto_at")
    ...
    df = pd.DataFrame(rows, columns=["primer_ad_id", "primer_contacto_at"])
    if not df.empty:
...
if __name__ == "__main__":
    ...
    if not ventas.empty:
        print(f"Monto total: {ventas['valor'].sum():,.2f}")
